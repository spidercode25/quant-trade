...
describe('VcpIndicators', () => {
  ...
  test('calculateORHigh returns the highest intraday high', () => {
    const candles = [
      { open: 100, high: 101, low: 99, close: 100.5 },
      { open: 100.5, high: 103, low: 100, close: 102.5 },
      { open: 102.5, high: 102.8, low: 101.5, close: 102 },
    ];

    expect(calculateORHigh(candles)).toBe(103);
  });

  test('calculateORHigh returns zero for empty candles', () => {
    expect(calculateORHigh([])).toBe(0);
  });
});
